...
const initializeTransporter = async () => {
  if (process.env.GMAIL_APP_PASSWORD) {
    ...
    // Gmail 설정이 없는 경우 Ethereal Email 사용 (테스트용)
    console.log('📧 Ethereal Email로 테스트 모드 초기화');
...
// 관리자 알림 이메일 생성 함수
const createAdminNotificationEmail = (userData) => {
  const { email, gender, age, occupation } = userData;
  ...
  return {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <h2 style="color: #333;">새로운 파일럿이 등록되었습니다!</h2>
        
        <div style="background: #f5f5f5; padding: 15px; border-radius: 8px; margin: 20px 0;">
          <h3 style="margin-top: 0; color: #555;">등록 정보</h3>
          <p><strong>이메일:</strong> ${email}</p>
          <p><strong>성별:</strong> ${gender === 'male' ? '남성' : '여성'}</p>
          <p><strong>나이:</strong> ${age}세</p>
          <p><strong>직업:</strong> ${occupation}</p>
          <p><strong>등록 시간:</strong> ${new Date().toLocaleString('ko-KR')}</p>
        </div>
        
        <p style="color: #666; font-size: 14px;">
          이 알림은 Pylot 파일럿 등록 시스템에서 자동으로 발송되었습니다.
        </p>
      </div>
    `
  };
  ...
  try {
    ...
    if (!email || !gender || !age || !occupation) {
      return res.status(400).json({
        ...
        required: ['email', 'gender', 'age', 'occupation']
      });
    }
    ...
    console.log('📧 파일럿 등록 요청:', { email, gender, age, occupation });
    ...
    const mailOptions = {
      ...
      to: email,
      ...emailContent // subject, html, attachments 포함
    };
    ...
    // 관리자 알림 이메일 발송
    const adminEmailContent = createAdminNotificationEmail({ email, gender, age, occupation });
    ...
    console.log('✅ 관리자 알림 이메일 발송 완료:', adminInfo.messageId);

    // 성공 응답
    res.status(200).json({
      success: true,
      message: '파일럿 등록이 완료되었습니다',
      data: {
        email,
